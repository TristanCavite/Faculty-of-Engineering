...
<template>
  <div class="flex items-center gap-3">
    <label class="hidden text-sm font-medium text-gray-700 sm:inline">Status:</label>
    ...
    <div class="relative inline-block">
      <DropdownMenu>
        ...
        <!-- same anchored dropdown pattern you used for EventFilter -->
...
import { ListFilter, CheckCircle2, FileText } from "lucide-vue-next";
import { computed } from "vue";

type StatusValue = "all" | "published" | "draft";
type Option = { value: StatusValue | string; label: string; icon: any };
...
const opts = computed<Option[]>(() => props.options?.length ? props.options : DEFAULT_OPTIONS);
const selected = computed(() => props.modelValue ?? "all");
const selectedLabel = computed(() => opts.value.find(o => o.value === selected.value)?.label ?? "All");
const selectedIcon  = computed(() => opts.value.find(o => o.value === selected.value)?.icon  ?? ListFilter);
